...
from datasets.video_instruction_dataset_VPTeA import Video_Instruct_Dataset
import math
import torch
...
def split_model(model_name):
    ...
    num_layers = {
        'InternVL3-1B': 24, 'InternVL3-2B': 24, 'InternVL3-4B': 32, 'InternVL3-8B': 32}[model_name]
...
class VideoAnomalyDetectionModel(pl.LightningModule):
    ...
    def forward(self, pixel_values, num_patches_list, batch_idx=0):
        ...
        pixel_values = torch.reshape(
            pixel_values,
            (batch_size * frame_size, pixel_values.size(2), pixel_values.size(3), pixel_values.size(4)),
        )
        ...
        responses = self.model.batch_chat(
            self.tokenizer,
            pixel_values,
            num_patches_list=num_patches_list_real,
            questions=questions,
    ...
    def training_step(self, batch, batch_idx):
        self.model.eval()
        if len(batch) == 5:
            pixel_values, num_patches_list, labels, video_name, cls_labels = batch
        elif len(batch) == 4:
            pixel_values, num_patches_list, labels, video_name = batch
            cls_labels = None
        else:
            raise ValueError(f"unexpected InternVL batch with {len(batch)} fields")
        labels = torch.as_tensor(labels, dtype=torch.long, device=pixel_values.device)
        true_label_sets = _sample_label_sets(cls_labels, len(labels))

        responses, pixel_values, num_patches_list_real = self.forward(
            pixel_values, num_patches_list, batch_idx
        )
        ...
        if not scope:
            ...
            optimizer_prompt = render_optimizer_prompt(
                self.optimizer_instruct, frame_prefix, sample_records, scope, self.kb
            )
            response = self.model.chat(
                self.tokenizer, pixel_values, optimizer_prompt, self.generation_config
            )
            try:
        ...
        predict_labels = torch.tensor(predict_labels, device=pixel_values.device)
        ...
        return torch.tensor(1.0, requires_grad=True, device=pixel_values.device) 

     
    def validation_step(self, batch, batch_idx):
        if len(batch) == 5:
            pixel_values, num_patches_list, labels, video_name, _ = batch
        elif len(batch) == 4:
            pixel_values, num_patches_list, labels, video_name = batch
        else:
            raise ValueError(f"unexpected InternVL batch with {len(batch)} fields")
        labels = torch.as_tensor(labels, dtype=torch.long, device=pixel_values.device)
        responses, pixel_values, _ = self.forward(pixel_values, num_patches_list, batch_idx)
        predict_labels = torch.tensor(
            [_binary_prediction(response) for response in responses], device=pixel_values.device
        )
        correct_predictions = (predict_labels == labels).sum()
        accuracy = correct_predictions / len(labels)
...
text_file = './stage1_generated_kb.txt'

f = open(text_file, 'w')
f.writelines('Generative Knowledge Base\n')
f.close()
...
# Create the datasets; please put the UCF frames in the configured UCF frame root.
# train_ann_root = 'Data/UCF_Instruct_train.json'
# train_dataset = Video_Instruct_Dataset(
#     vis_root='G:/ucf/', ann_root=train_ann_root, num_sampled_frame=8
# )
# val_dataset = Video_Instruct_Dataset(
#     vis_root='G:/ucf/', ann_root='Data/UCF_Instruct_val.json',
...
train_dataset = Video_Instruct_Dataset(
    vis_root=xd_frame_root, ann_root=train_ann_root, num_sampled_frame=8
)
val_dataset = Video_Instruct_Dataset(
    vis_root=xd_frame_root, ann_root=val_ann_root,
    TEST_FLAG=True, num_sampled_frame=8
)
...
train_loader = DataLoader(train_dataset, batch_size=2, shuffle=True, num_workers=0, drop_last=False, collate_fn=VPTeA_collate)
val_loader = DataLoader(val_dataset, batch_size=2, shuffle=False, num_workers=0, drop_last=False, collate_fn=VPTeA_collate)
...
lightning_model = VideoAnomalyDetectionModel(
    ...
    model_instruct=model_instruct, generation_config=generation_config,
    knowledge_base=knowledge_base,
)

from pytorch_lightning.loggers import TensorBoardLogger
tb_logger = TensorBoardLogger(save_dir="logs_VPTeA/")
...
trainer = pl.Trainer(
    ...
    strategy="auto"  # Distributed Data Parallel training
)
