...
# Prompt pairs are loaded from the annotation-derived knowledge base.
...
class VideoAnomalyDetectionModelInference(pl.LightningModule):
    ...
    def forward(self, pixel_values, num_patches_list):
        ...
        question = question.replace(
            '[$KnowledgeBase]',
            json.dumps(self.kb.render_all(), ensure_ascii=False, indent=2),
        )

        questions = [question] * batch_size
        num_patches_list_real = [frame_size] * batch_size

        responses = self.model.batch_chat(self.tokenizer, pixel_values, num_patches_list=num_patches_list_real, questions=questions, generation_config=self.generation_config)
...
def main():
    ...
    test_dataset = Video_Instruct_Dataset_Inference(vis_root='G:/ucf/frames/', ann_root='Data/UCF_Eval.json')
    ...
    knowledge_base = load_or_create_knowledge_base(
        test_dataset.annotation, 'Data/UCF_Eval.json'
    )
    ...
    while True:
        ...
        try:
            ...
            for ind in indices:
                ...
                dict_single[ind]={'start':snippet_start,'end':snippet_end, 'score':predict_labels[0]}
