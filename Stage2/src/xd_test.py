...
from utils.tools import get_batch_mask, load_xd_prompt_pairs, XD_CLASS_NAMES
...
def evaluate(model, testdataloader, maxlen, positive_prompts, negative_prompts, gt, gtsegments, gtlabels, device, map_gate_gamma=0.5):
    model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        for item in testdataloader:
            ...
            for index in range(len(lengths)):
                lengths[index] = min(length, maxlen)
    ...
    return visual_auc, pair_ap, average_map
